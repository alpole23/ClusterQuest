...
> **Note:** Detection is hardcoded to phosphonate BGCs only. KnownClusterBlast (`--cb-knownclusters`), `--clusterhmmer`, and `--tigrfam` are always enabled, and `--no-zip-output` suppresses the redundant per-genome `.zip`. The whole-genome summary GenBank is off unless you pass `--antismash_summary_gbk`.
